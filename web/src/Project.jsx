  ...
  const ctx = useMemo(() => ({
    ...
    onDiagnostic: (test, args) => act(() =>
      rt.call("diagnostic", { round_id: round, test, project: pid, session: sessionId,
                              ...args })),
    onDiagnose: (proposal && round === proposal.round) ? () => act(() => {
      rt.call("propose", {
        round_id: round, project: pid, session: sessionId,
        payload: {
    ...
    refresh,
  }), [pid, view, campaign, round, roundView, sessionId, artifacts, busy, error, onTrace,
       traced, lineage, drops, dropNotes, proposal, act, refresh]);
