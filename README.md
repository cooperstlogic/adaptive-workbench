...
.venv/bin/python check.py      # 48 invariant checks, all should pass
```
...
.venv/bin/python simulate_campaign.py        # run the campaign and render the chart, ~35 s
.venv/bin/python plot_campaign.py            # re-render the chart alone, ~1 s
.venv/bin/python check.py                    # verify every documented invariant
```

`check.py` runs 48 checks and is the handoff contract. Every check in it corresponds to a rule in
...
in 12 of 20 seeds, a median 0.605 pKD worse — a four-fold error in the KD of the lead
...
firing: 20 of 20 at round 5 and 11 of 20 at round 6, against 6 and 0 for the corrected
arm.
...
| Naive pooling | does **not** degrade selection. It degrades the ranking you act on: a worse lead in 12 of 20 seeds, median 0.605 pKD |
...
| `check.py` | Invariant verification, 48 checks. Run it after any phase |
