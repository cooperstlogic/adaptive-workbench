...
| --- | --- | --- | --- |
| Mean rounds to threshold | **2.35** | 4.20 | 2.35 |
| Median rounds to threshold | **2.0** | 3.5 | 2.0 |
| Reached threshold by round 6 | 20/20 | 14/20 | 19/20 |
...
threshold after seeing a curve would make the chart worthless. **Every parameter below is still
the pre-registered one except the gate**, which was amended on 2026-09-20 after the campaign
had run — see [Amendment: the threshold rule](#amendment-the-threshold-rule) immediately
below. The landscape itself did not move: `landscape.npz` and `oracle.bin` are byte-for-byte
what `473ac01` produced.
...
| **Threshold (the gate)** | ~~99th percentile → 10.762~~ **amended to 75% of the parent-to-max climb → 11.006** | Pre-registered as the 99th percentile and fixed before the first run; **superseded after the fact** — see the amendment below |
...
run: **7,294 feasible** designs of 10,261, solved β 0.5527, realized linear R² **0.6008**
against the 0.60 target, cliff at VH 103 (parent residue F, selected by the pre-registered
rule), detection limit **6.861**, and — as pre-registered — **threshold 10.762** with 74 of
