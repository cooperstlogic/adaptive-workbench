...
had ever run.** Git history is the evidence the order was kept: `8f13010` (pre-register) →
`473ac01` (generate the landscape) → `945277c` (first campaign). Every value carries a
justification that does not reference the outcome, because picking a threshold after seeing a
curve would make the chart worthless. **The threshold is fixed at 10.762 pKD and is never
recomputed.**
...
references in source comments point at a build decision log that was removed once the build
finished; it remains in git history.
