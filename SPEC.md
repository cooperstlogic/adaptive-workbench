...
> - **Decisions are buttons; questions are asks.** Approval and the four ruling verbs stay outside the composer. Reads — status, why a round flagged, whether the lab has reported — are contextual suggested asks over it, offered only when the state gives a reason to suggest one, presented as a choice and sent to the model in the seat (decision 158); without a seat they are answered by a deterministic briefing assembled in `wb_driver` from artifacts on disk, every figure naming its `core/` function.
...
| Home, Projects and Recent sessions | Projects and recent rounds. `+ New project` opens the dialog — name, description, instructions, and above them *from a template* or *blank*; the first hands off to the template library (decision 168) | Demo beat 1, in its native slot |
...
**Anyone with the link can use it, until the budget says otherwise.** The key lives in a Vercel environment variable, so visitors supply nothing but the link, which carries an access code the page keeps and sends on every call; without it the site is in replay and says so (decision 160). A global daily spend cap, a per-IP counter and a small in-flight count live in Upstash Redis, provisioned from the Vercel marketplace, and a call reserves its maximum from the day before it is made rather than writing its cost after (decision 161). Under the cap the session runs live. Over it, the site returns to replay — and the fallback is not a degraded mode, it is the default that live calls temporarily upgrade. The badge reads *live*, or *replayed · evidence recomputed · 5 of 5 values match*, and the page always says which mode it is in.
...
1. **Thirty seconds.** The home screen: one card reading *Round 4 — ruling pending*, a project list, and behind `+ New project` a dialog whose first control is *from a template* or *blank*. Take the template library, pick antibody affinity maturation, name the lead, and a project exists. This is the part Claude Science has no answer for today — and the screen it is missing from is its own.
...
**The public panel runs up a bill.** Covered by the access code in the link, the daily cap reserved before each call, the per-IP counter, and by the function never accepting a caller-supplied message array. Verify all of it before the URL is shared, not after.
...
| The laboratory's turnaround | **Staged, and movable by hand — labelled *simulated* where it sits.** A submitted round is held: the first check comes back with the date the registry expects and the pull is refused. *Have the lab report now* calls `lims.py release`, which marks the run reported and records who said so. It moves when the rows are handed over and nothing about what they say — the oracle measured them at submission — and the round still has to be pulled by someone asking. It is not on the registry's tool list, because a registry does not have a button that finishes an assay (decision 163) |
...
| The push-back round trip | **Real, both ways.** A live pass continues the session's signed transcript, from the last question asked if one was; a replayed pass answers with the recorded pass 2. The committed round-4 record is two-pass: d.webster sent it back naming `residual_by_plate`, and a second headless gate answered — `gates/pushback-round4.md` |
| The agent's reasoning in the CLI | **Real.** Claude Code runs the diagnosis unaided; this is acceptance criterion 3 |
| A past round's session | **Real, and read rather than retold.** The centre column renders this browser's command log; a round that ran before the tab was opened has none, so its session is assembled from the artifacts it wrote — the batch, the approval stamp, the submission, the reconciliation, the anomaly verdict, the frame move, the fit. No command chip appears in it, because no command ran here, and the files it was read from are named underneath (decision 162) |
...
| The proof chart | **Real, and the evaluator's.** Twenty simulated campaigns per arm on the synthetic landscape, drawn as the template's Validation on the Objectives tab and the template library's configure screen. It never appears on a project's own Progress chart, which draws only what that project measured (decision 135) |
| The composer's free text | **Real when a live seat is available, and a follow-up is a follow-up.** Free text goes to the model with the project's state as context, the two read tools and the registry check, on the session's signed transcript, so it can refer to what was said earlier in the session; the transcript lives in page memory and a reload starts a fresh one from the record. The suggested asks appear over it only when the project's state gives a reason — a round at the lab or one whose results are in and unpulled, a flagged round nobody has ruled on, a round that came back quiet and has not been carried forward yet (decision 165) — presented as a choice headed by that reason, each option showing the prompt it sends, then *Let the agent decide*; they go to the same seat, and reappear when the state changes what there is to suggest. The results check is the one that is a call to the registry instead, and it says so. Without a seat the box says so in a phrase and the asks are answered from artifacts on disk by the briefing, with nothing over the answer |
